...
yd1 = [];
for i = 1:size(t)
    yd1p = (-pi/2+(50*pi/180)*(1-cos(0.63*t(i))))*(t(i)<=5) + (10*pi/180)*(t(i)>5);
    yd1 = [yd1;yd1p];
end
plot(t,yd1)
...
yd2 = [];
for i = 1:size(t)
    yd2p = ((170*pi/180)-(60*pi/180)*(1-cos(0.63*t(i))))*(t(i)<=5)+(50*pi/180)*(t(i)>5);
    yd2 = [yd2;yd2p];
end
plot(t,yd2)
...
yd1dot = [];
for i = 1:size(t)
    yd1v = ((31.5*pi/180)*sin(0.63*t(i)))*(t(i)<5)+ 0*(t(i)>=5);
    yd1dot = [yd1dot;yd1v];
end
plot(t,yd1dot)
...
yd2dot = [];
for i = 1:size(t)
    yd2v = ((-37.8*pi/180)*sin(0.63*t(i)))*(t(i)<5)+ 0*(t(i)>=5);
    yd2dot = [yd2dot;yd2v];
end
plot(t,yd2dot)
...
yd2dotdot = [];
for i = 1:size(t)
    yd1a = ((19.845*pi/180)*cos(0.63*t(i)))*(t(i)<5)+ 0*(t(i)>=5);
    yd2a = ((-23.814*pi/180)*cos(0.63*t(i)))*(t(i)<5)+0*(t(i)>=5);
